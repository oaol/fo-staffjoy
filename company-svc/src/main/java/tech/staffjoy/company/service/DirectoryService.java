...
import tech.staffjoy.company.dto.Association;
...
import tech.staffjoy.company.repository.DirectoryRepo;
...
//    static final ILogger logger = SLoggerFactory.getLogger(DirectoryService.class);
    ...
    public DirectoryEntryDto createDirectory(NewDirectoryEntry req) {
        ...
        GetOrCreateRequest getOrCreateRequest = GetOrCreateRequest.builder()
                ...
                .build();
        ResponseEntity<AccountDto> geOrCreateAccountResponse = null;
        try {
            geOrCreateAccountResponse = accountClient.getOrCreateAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, getOrCreateRequest);
        } catch (Exception ex) {
            String errMsg = "could not get or create user";
//            serviceHelper.handleErrorAndThrowException(logger, ex, errMsg);
        }
        if (geOrCreateAccountResponse.getStatusCode().isError()) {
//            serviceHelper.handleErrorAndThrowException(logger, genericAccountResponse.getMessage());
        }

        AccountDto account = geOrCreateAccountResponse.getBody();
...
//        serviceHelper.onboardWorkerAsync(onboardWorkerRequest);
    ...
    public DirectoryList listDirectory(String companyId, int offset, int limit) {
        ...
        for(Directory directory : directoryPage.getContent()) {
            ...
            try {
                accountResponse = accountClient.getAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, directoryEntryDto.getUserId());
            } catch (Exception ex) {
...
//                serviceHelper.handleErrorAndThrowException(logger, resp.getMessage());
    ...
    public DirectoryEntryDto getDirectoryEntry(String companyId, String userId) {
        ...
        if (directory == null) {
            throw new ServiceException("directory entry not found for user in this company");
        }
        directoryEntryDto.setInternalId(directory.getInternalId());

        ResponseEntity<AccountDto> accountResponse = null;
        try {
            accountResponse = accountClient.getAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, userId);
        } catch (Exception ex) {
...
//        }
    ...
    public DirectoryEntryDto updateDirectoryEntry(DirectoryEntryDto request) {
        ...
        ResponseEntity<AccountDto> accountResponse = null;
        try {
            accountResponse = accountClient.getAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, orig.getUserId());
        } catch (Exception ex) {
...
//            serviceHelper.handleErrorAndThrowException(logger, genericAccountResponse1.getMessage());
        ...
        if(account.isConfirmedAndActive() && accountUpdateRequested) {
            throw new ServiceException("this user is active, so they cannot be modified");
        } else if (account.isSupport() && accountUpdateRequested) {
            throw new ServiceException("you cannot change this account");
        }
        ...
        if (accountUpdateRequested) {
            ...
            ResponseEntity<AccountDto> updateAccountReponse = null;
            try {
                updateAccountReponse = accountClient.updateAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, account);
            } catch (Exception ex) {
                String errMsg = "view updating account";
//                serviceHelper.handleErrorAndThrowException(logger, ex, errMsg);
            }

            if (updateAccountReponse.getStatusCode().isError()) {

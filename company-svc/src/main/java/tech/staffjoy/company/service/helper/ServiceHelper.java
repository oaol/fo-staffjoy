...
import tech.staffjoy.company.config.AppConfig;
...
//    private SentryClient sentryClient;
    ...
    public void trackEventAsync(String event) {
        ...
        ResponseEntity<String> trackEventResponse = null;
        try {
            trackEventResponse = accountClient.trackEvent(trackEventRequest);
        } catch (Exception ex) {
            String errMsg = "fail to trackEvent through accountClient";
//            handleErrorAndThrowException(logger, ex, errMsg);
        }
        if (trackEventResponse.getStatusCode().isError()) {
